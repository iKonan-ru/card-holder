...
describe('useImportCards', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(importUtils.parseImportedFile).mockReturnValue(mockPayload);
    vi.mocked(importUtils.validateImportedPayload).mockReturnValue(undefined);
    ...
    vi.mocked(errorUtils.createImportSuccessMessage).mockReturnValue(
  ...
  it('должен парсить и валидировать payload', async () => {
    ...
    expect(importUtils.parseImportedFile).toHaveBeenCalledWith(
      MOCK_FILE_CONTENT
    );
    expect(importUtils.validateImportedPayload).toHaveBeenCalledWith(
      mockPayload
    );
  });
  ...
  it('должен парсить расшифрованные карты', async () => {
    ...
    await waitFor(() => {
      expect(importUtils.parseDecryptedCards).toHaveBeenCalledWith('[]');
    });
  });
  ...
  it('должен объединять карты', async () => {
    ...
    await waitFor(() => {
      expect(importUtils.mergeCards).toHaveBeenCalledWith(
        mockCards,
        mockImportedCards
      );
    });
